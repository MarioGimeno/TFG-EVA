...
// Configuración de Multer para almacenar archivos en disco (en TMPDIR)
 ...
 * Se le pasan dos rutas: la del archivo encriptado (inputFilePath) y la ruta destino para el archivo desencriptado (outputFilePath).
 */
 ...
 * Se procesan los archivos usando Worker Threads para desencriptarlos y streams para subirlos a Google Cloud Storage.
 */
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Procesar el archivo de ubicación (se usa el primer archivo recibido)
      ...
      const uploadTasks = await Promise.all(videoFiles.map(async (file) => {
        ...
        // Borrar archivos temporales: se elimina el archivo encriptado, el desencriptado y el de ubicación.
      ...
      // Borrar el archivo de ubicación desencriptado (usado para todos)
